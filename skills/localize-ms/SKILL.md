---
name: localize-ms
description: "Use when translating or localizing UI strings, marketing copy, documentation, or any source text into Malay (Bahasa Melayu) for Malaysia, Brunei, and Singapore. Malay is NOT Indonesian — different tech vocabulary (muat naik/muat turun/tetapan not unggah/unduh/pengaturan), 1,234.56 format, RM Ringgit, and dangerous false friends (butuh, pusing, sulit differ). Enforces the affix system with nasal assimilation, mandatory -kan on transitives, Anda/awak formality, bahasa baku over slang, the file-browse (semak imbas) vs web-surf (melayari) split, and Islamic/multiracial sensitivity."
---
