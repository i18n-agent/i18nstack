---
name: localize-th
description: Use when translating or localizing UI strings, marketing copy, documentation, or any source text into Thai (ภาษาไทย). Isolating Tai-Kadai — NO gender, number, case, or conjugation. The defining feature is NO SPACES between words (spaces only at clause breaks). Enforces the 5-tone system with mandatory tone marks, the classifier system (คน/ตัว/อัน/แผ่น), three formality registers (polite สุภาพ as UI default), particles ครับ/ค่ะ never in buttons, คุณ pronoun, modifier-after-head order (ไฟล์ใหม่), Buddhist Era date option, ฿ Baht, and list parallelism.
---
