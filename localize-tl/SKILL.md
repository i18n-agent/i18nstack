---
name: localize-tl
description: Use when translating or localizing UI strings, marketing copy, documentation, or any source text into Tagalog/Filipino (Wikang Filipino). Austronesian, NOT Spanish despite loanwords. The defining feature is the focus/trigger system (verb affix selects the ang-phrase; aspect via CV-reduplication, not tense). Enforces verb-initial word order, case markers ang/ng/sa, mga before nouns (omitted with numbers), na/-ng ligature, NO gender, po/opo politeness particles, inclusive/exclusive we (kami vs tayo), i- verbalizer for English roots (i-save, i-click), Spanish month/day names, ₱ Peso (₱1,234.56), and pure Filipino for formal UI.
---
